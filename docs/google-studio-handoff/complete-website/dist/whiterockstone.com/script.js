...
function openColor(slug) {
  ...
  modal.querySelector("#colorRelatedProducts").href = `products.html?q=${encodeURIComponent(color.material)}`;
...
function renderApplications() {
  ...
  container.innerHTML = (applications.items || []).map((item) => {
    ...
    return `<article class="application-card"><figure>${renderPicture(item, "image", `${item.title}${isRender ? " illustrative render" : ""}`)}${isRender ? '<figcaption>Illustrative render — not an actual WHITEROCK project.</figcaption>' : ""}</figure><div><p class="eyebrow">${escapeHtml(item.category)} · ${escapeHtml(item.featuredColor)}</p><h3>${escapeHtml(item.title)}</h3><p>${escapeHtml(item.description)}</p><a class="text-link" href="${colorHref}">View the featured color →</a></div></article>`;
  }).join("");
}
